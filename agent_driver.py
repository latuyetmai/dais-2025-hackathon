...
# MAGIC
# MAGIC system_prompt = """
# MAGIC You are an agent that provides information for places based on wheelchair accesibility, you have access to the following tools:
# MAGIC 1) google_maps_acc
# MAGIC    - Pick this tool for other bussiness that are not hotels or airbnb
# MAGIC    - Consider that the results are sorted by rating in desc order
# MAGIC    - when using this tool return the result in a table in markdown format, include the city, name, phone number, address, rating and place_url
# MAGIC 2) hotel_acc
# MAGIC    - Pick this tool for hotels
# MAGIC 3) airbnb_vindex
# MAGIC    - Pick this tool for airbnb
# MAGIC    - if the question asks for reviews then use full_text
# MAGIC    - render the name, location, url, full_text
# MAGIC    - do not pass fiilters
# MAGIC """
# MAGIC
# MAGIC ###############################################################################
...
# MAGIC uc_tool_names = ["workspace.acc.google_maps_acc","workspace.acc.hotel_acc"]
...
# MAGIC     VectorSearchRetrieverTool(
# MAGIC         index_name="workspace.acc.airbnb_vindex",
# MAGIC         # TODO: specify index description for better agent tool selection
# MAGIC         tool_description="Provides airbnb description and reviews based on wheelchair/accessible properties",
# MAGIC         filter=None
# MAGIC     )
# MAGIC ]
...
for event in AGENT.predict_stream(
    {"messages": [{"role": "user", "content": "return the accessible airbnb in san diego"}]}
):
    print(event, "-----------\n")
...
input_example = {
    "messages": [
        {
            "role": "user",
            "content": "return the top ten restaurants in New York that are restaurant accessible"
        }
    ]
}
