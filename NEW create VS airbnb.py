...
# Load data
df = spark.sql("""
    SELECT id, property_id, location, final_url, full_text
...
# Save to Delta table
chunk_df.write.option("mergeSchema", "true").mode("overwrite").saveAsTable("workspace.acc.airbnb_chunks")
...
vsc.create_delta_sync_index(
    ...
    # metadata_columns=["final_url", "location"]
)
...
results = index.search(
    query="wheelchair accessible home near Disney",
    search_kwargs={
        "k": 5,
        "query_type": "hybrid",
        "filter": {"property_id": ["46727535", "13080625"]}
